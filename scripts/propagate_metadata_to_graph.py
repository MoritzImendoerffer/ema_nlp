...
(``:Document.doc_type`` / ``.audience`` / ``.site_topic``) from the canonical
...
refreshed (new scrape, new JSON export) and the graph should pick them up
without rebuilding. Safe to re-run: each label group is SET from its row
(badge nulls overwrite, clearing labels a page lost; ``doc_type`` is only
touched on rows the export pass stamped).
...
def load_rows(client: MongoClient[Any], wanted: set[str]) -> tuple[list[dict], list[dict]]:
    """Split enrichment rows for indexed docs into (badge_rows, doc_type_rows).
    ...
    cursor = col.find(
        {},
        {"doc_id": 1, "doc_type": 1, "audience": 1, "site_topic": 1, "provenance": 1},
    ...
    try:
        ...
        cursor.close()
    return badge_rows, doc_type_rows
...
def propagate(*, batch_size: int = 5000, dry_run: bool = False) -> None:
    ...
    try:
        badge_rows, doc_type_rows = load_rows(client, wanted)
    finally:
        client.close()
    ...
    print(
        ...
        f"doc_type {len(doc_type_rows)} / {len(wanted)} indexed docs"
    )
    ...
    for t, n in Counter(r["doc_type"] or "(empty)" for r in doc_type_rows).most_common(30):
        print(f"  {t:36s} {n}")
    if not badge_rows and not doc_type_rows:
