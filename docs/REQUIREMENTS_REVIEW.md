...
| F3 | 🔴 | Offline `mlflow.genai` faithfulness judge never sees context (`{{context}}`→`inputs`; predict_fn returns no passages) — scores meaningless; inline path OK | `harness/eval/judges.py:26-31`, `harness/eval/predict.py:34-42` |
| F4 | 🔴 | Rated-trajectory cache: per-session `QueryCache()` + full-file rewrite on save → cross-session lost updates of entries and ratings (the few-shot learning signal) | `app.py:360`, `harness/query_cache.py:80-86` |
| F5 | 🔴 | Eval cannot consume recipes: `build_predict_fn` needs `.run`/callable; adapter has neither; only recipe-bypassing `build_session` works | `harness/eval/predict.py:32`, `harness/agents/workflow_adapter.py` |
| F6 | 🟠 | Two divergent composition paths: `build_session` (agent YAML, pipeline=`native` hardcoded) vs `build_recipe` (recipes, pipeline per recipe) | `harness/agents/session.py:105-140` |
| F7 | 🔴 | Few-shot injection near-unreachable: `min_examples` default 3 never overridden; `k<3` recipes can never inject; not tunable from `FewshotPolicy` | `app.py:590`, `harness/fewshot_inject.py:51,69` |
...
| F17 | 🟡 | `corrective_search` returns the *last* retrieval, not best-so-far (comment claims otherwise); grader prompt demands `qa_id`s absent from its context; parse-error pseudo-fact fed to rewriter | `harness/retrieval/corrective.py:26,38-45,81`, `harness/tools/corrective_search.py:100-113` |
...
| F19 | 🟡 | UI: cache toggle off silently kills few-shot + rating persistence; 👎-rated entries offered for reuse; `on_chat_resume` reverts to default recipe; failed profile switch leaves UI/pipeline disagreeing | `app.py:490-494,533,418,474-477` |
| F20 | 🟡 | Dead artifacts/deps: `scripts/generate_comparison_report.py` (consumes deleted engine's outputs), issue-generator scripts with pre-refactor plans, `ablations/a1_query_expansion.py` orphaned, empty `main.py`, unused deps (`llama-index-vector-stores-faiss`, `llama-index-retrievers-bm25`, `rank-bm25`), empty `harness/{workflows,pg,hitl}/` dirs (mask import errors as namespace pkgs), ~10 stale `WorkflowRunner` comments, `HARNESS_REFACTORS.md` is a Phoenix-era spec | various (audit 2026-07-02) |
